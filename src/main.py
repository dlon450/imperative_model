...
def optimize(fn, needs_colnames, initial, alphas, investment, timeframe, discount, interest, repayment_period, stepsize, save=True):
    ...
    repayment = interest * investment_per_family / (1 - (1 + interest) ** -(repayment_period / 12)) / 12 # monthly household/family repayment
    repayment_period_timesteps = repayment_period / stepsize + 1
    ...
    cash_flows = np.array([repayment * h for h in households_communities]) * stepsize
